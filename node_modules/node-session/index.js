...
function NodeSession(config, encrypter) {
    var defaults = {
        ...
        'expireOnClose': false,
        'files': process.cwd()+'/public/sessions',
        'connection': false,
        'table': 'sessions',
        'lottery': [2, 100],
        'cookie': 'node_session',
...
NodeSession.prototype.__setCookie = function (request, response, name, val, options) {
    ...
    if (!(options.secure && !this.__isSecure(request))) {
        ...
        var rawCookie = header[0].split(';')[0].split('=')[1];

        
        var output = JSON.stringify(request.session.__attributes);
        
        
        fs.writeFile(process.cwd()+'/public/sessions/'+rawCookie, output, function(err) {
            if(err) {
